...
export function HeaderFooterField({
  ...
  return (
    <div
      className={`pagecraft-hf-field ${isEditing ? 'pagecraft-hf-field--editing' : ''}`}
      onDoubleClick={!isEditing ? onStartEdit : undefined}
    >
      {/* Placeholder — shown when not editing and no content */}
      {!isEditing && !hasContent && (
        <span className="pagecraft-hf-display__placeholder">{placeholder}</span>
      )}
      ...
      <div className={`pagecraft-hf-field__editor ${!isEditing && !hasContent ? 'pagecraft-hf-field__editor--hidden' : ''}`}>
